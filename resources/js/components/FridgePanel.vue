...
<template>
    <div class="flex min-h-0 flex-1 flex-col overflow-hidden px-4 pt-4" :aria-busy="fridgeLoading || actionLoading">
        <div v-if="showHeading" class="flex shrink-0 items-start justify-between gap-3">
            <div class="min-w-0">
                <h2 class="text-lg font-semibold text-slate-950">Что у вас сейчас есть</h2>
                <p class="mt-0.5 text-pretty text-sm text-slate-500">Доставленные блюда и остатки порций</p>
            </div>
        ...
        <div v-else class="mt-4 min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain pb-5 pr-1">
            <article
                ...
                :key="item.id"
                class="rounded-2xl bg-slate-50/80 p-4 ring-1 ring-inset ring-slate-100"
            >
                <div class="flex items-start justify-between gap-3">
                    ...
                    <Badge variant="outline" class="shrink-0 rounded-full border-slate-200 bg-white px-3 text-xs font-semibold tabular-nums text-slate-600">
                        {{ item.quantity_remaining }} шт.
                    </Badge>
                </div>
                <p class="mt-3 rounded-xl bg-white px-3 py-2 text-xs font-medium tabular-nums text-slate-600 ring-1 ring-inset ring-slate-100">{{ expiryLabel(item.expires_at) }}</p>
                ...
                <div class="mt-4 grid grid-cols-2 gap-2">
                    <Button
                        ...
                        variant="outline"
                        size="sm"
                        class="h-10 rounded-full border-slate-200 bg-white text-xs font-semibold text-slate-700 transition-[background-color,transform] duration-150 hover:bg-slate-50 active:scale-[0.98]"
                    ...
                    <AlertDialogRoot>
                        <AlertDialogTrigger as-child>
                            <Button
                                ...
                                variant="outline"
                                size="sm"
                                class="col-span-2 h-10 w-full rounded-full border-transparent bg-transparent text-xs font-semibold text-rose-700 transition-[background-color,transform] duration-150 hover:bg-rose-50 active:scale-[0.98]"
                                :disabled="actionLoading"
                            >
                                Выбросить
                            </Button>
