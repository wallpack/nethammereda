...
class DemoDatabaseSeeder extends Seeder
{
    ...
    {
        if (! config('lunch.demo_reset_execution_authorized', false)) {
            throw new LogicException('DemoDatabaseSeeder may only be executed through php artisan demo:reset.');
        }
        ...
        [$businessWeekStart, $startsAt, $closesAt] = $this->orderWindow();
        $cycle = OrderCycle::query()->create([
            'title' => 'Неделя '.$businessWeekStart->format('d.m.Y'),
            'starts_at' => $startsAt,
            'closes_at' => $closesAt,
     ...
     */
    private function orderWindow(): array
    {
        $businessTimezone = (string) config('lunch.business_timezone', 'Asia/Yekaterinburg');
        $deadlineTime = (string) config('lunch.order_deadline_time', '17:00');
        $deadlineClock = CarbonImmutable::createFromFormat('!H:i', $deadlineTime, $businessTimezone);

        if ($deadlineClock === false || $deadlineClock->format('H:i') !== $deadlineTime) {
            throw new LogicException('LUNCH_ORDER_DEADLINE_TIME must use HH:MM 24-hour format.');
        }
        ...
        $closesAt = $weekStart
            ->addDays(4)
            ->setTime($deadlineClock->hour, $deadlineClock->minute);
        ...
        return [
            $weekStart,
            $weekStart->setTimezone($applicationTimezone),
            $closesAt->setTimezone($applicationTimezone),
        ];
    }
